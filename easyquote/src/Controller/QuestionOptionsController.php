...
class QuestionOptionsController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->QuestionOptions->save($questionOption)) {
                $this->Flash->success('The question option has been saved.');
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error('The question option could not be saved. Please, try again.');
            }
        }
        $questions = $this->QuestionOptions->Questions->find('list', ['limit' => 200]);
        $options = $this->QuestionOptions->Options->find('list', ['limit' => 200]);
        $this->set(compact('questionOption', 'questions', 'options'));
        $this->set('_serialize', ['questionOption']);
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->QuestionOptions->delete($questionOption)) {
            $this->Flash->success('The question option has been deleted.');
        } else {
            $this->Flash->error('The question option could not be deleted. Please, try again.');
        }
        return $this->redirect(['action' => 'index']);
    }
