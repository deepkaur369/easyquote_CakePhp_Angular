...
?>
<script>
	var q = Number(<?php echo $limit-100;?>);
	var o = Number(<?php echo $optionLimit-200;?>);	
...
<?php $quest=0; $ldiv=0; foreach($options as $option){
...
<input type="hidden"  id="lastdiv" value="<?php echo $ldiv; ?>" />
<div id="topBar" style="visibility:visible;">
	...
	<div style="width:50%;float:left;text-align:right">
		<a onclick="setCookie('js_status', 'True', 1)" href="#" style="display:none" id="editFlow" class="btn btn-success"> Enable Editing </a>
		<?php echo $this->Html->image('loader.gif',array('alt'=>'Wait!','id'=>'editloder')); ?>
	</div>
	

</div>
<br/><br/>
    <div class="demo statemachine-demo" id="statemachine-demo" style="visibility:visible;">
...
<!--##################### Stack for Questions #############################-->
			...
			<div style="width:20%!important;float:left!important;">
				...
				?>
				<div class="w q" style="left:560px;"  ondrag="getPositions(this)"  id="<?php echo $i ?>">
...
<!--######################### Stack for Options ##########################-->	
			...
			<div style="width:15%!important;float:right!important;text-align:right;">
				<?php for($i = $limit+10; $i<=$optionLimit-200;$i++){
					if(!in_array($i,$optn_ecpt)){
				?>
	...
	<?php 
		...
		foreach($questions as $question){
			...
			if(isset($question->relations[0]->option_id)){
				?>
	
	
	<!-- ###########################  Dynamic Question object creation  ########################### -->	
	
		<div class="w q" <?php echo $question->style; ?>  ondrag="getPositions(this)" id="<?php echo $i ?>">
			<?php
				foreach($question->options as $option){
				?>
				<button type="button" style="display:none;" id="queClose<?php echo $i ?>" style="margin-top: -19px;margin-right: -15px;" onclick="Closegetquediv(<?php echo $option->id; ?>,<?php $option->question_id; ?>,<?php echo $i; ?>)" class="close" >&times;</button>
				
				<?php } ?>
				
				<!-- Parents ids container -->
				<div id="parents<?php echo $i ?>">
				
					<?php foreach($question->relations as $relation){?>
				
					<input type="hidden" class="belongto<?php echo $relation->question_div; ?>" id="<?php echo $relation->option_div; ?>-<?php echo $relation->question_div; ?>" value="<?php echo $relation->option_div; ?>">
				...
				<!-- hidden container -->
				<input type="hidden"  id="type<?php echo $i ?>" value="question" />
				<input type="hidden" autocomplete="off" value="<?php echo $question->id; ?>" id="last_insert_id_<?php echo $i ?>" />
				<input type="hidden" id="parent_id_<?php echo $i ?>" value="<?php echo isset($question->relations[0]->option_id)?$question->relations[0]->option_id:''; ?>" />
				<input type="hidden" id="style_<?php echo $i ?>" value='<?php echo $question->style ?>' />
	
				<!-- Add Button  -->
				<a href="#question" id="add1<?php echo $i ?>" onclick="setID(<?php echo $i ?>,'question')" data-toggle="modal" data-target="#question" > </a>
				
				<a style="color:white;display:none;" id="adddv<?php echo $i ?>" href="javascript:getClk(<?php echo $i ?>)" >Add Questions</a>

				<!-- Update Button -->
				<span style="color:white;" id="pree<?php echo $i ?>">Q: &nbsp;</span>
			
				<a style="color:white;" href="#question" data-original-title="Question" data-content="<?php echo $question->question; ?>" data-placement="right" data-trigger="hover" class="popovers" id="edit<?php echo $i ?>" onclick="setID(<?php echo $i ?>,'question'),editQuestion(<?php echo $i ?>);" data-toggle="modal" data-target="#question" ><?php echo $question->title; ?></a>
	...
	<?php
			foreach($question->options as $option){
				...
				?>
	<!-- ###########################  Dynamic option object creation  ########################### -->				
				<div ondrag="getPositions(this)"  <?php echo $option->style; ?>  class="w" id="<?php echo $i ?>">
						<button type="button" style="display:none;" id="optClose<?php echo $i ?>" style="margin-top: -19px;margin-right: -15px;"  onclick="Closegetoptdiv(<?php echo $opts; ?>,<?php echo $option->question_id; ?>,<?php echo $i ?>)" class="close" >&times;</button>
						
						<!-- hidden container -->
						<input type="hidden" value="<?php echo $option->question_div; ?>" id="qdiv<?php echo $i ?>" />
						<input type="hidden"  id="type<?php echo $i ?>" value="option" />
						<input type="hidden" id="last_insert_id_<?php echo $i ?>" value="<?php echo $option->id; ?>" />
						<input type="hidden" id="parent_id_<?php echo $i ?>" value="<?php echo $option->question_id; ?>" />
						<input type="hidden" id="style_<?php echo $i ?>" value='<?php echo $option->style; ?>' />
					
						<a href="#option" id="clickMe_<?php echo $i ?>" onclick="setID(<?php echo $i ?>,'option')" data-toggle="modal" data-target="#option" ></a>
						
						<!-- Add Button -->
						<a id="add<?php echo $i ?>" style="display:none;" href="javascript:getClick(<?php echo $i ?>,'dummy')" >Add Options</a>
						<!-- Update Button -->
						<span style="color:black" id="pree<?php echo $i ?>">O:&nbsp; </span>	
						
						<a data-original-title="Option" data-content="<?php echo $option->name; ?>" data-placement="right" data-trigger="hover" class="popovers"  id="edit<?php echo $i ?>" href="javascript:getClick(<?php echo $i ?>,'getdata')" ><?php echo $option->title; ?> </a>
						
						
						<div class="ep" style="color:white" id="drag<?php echo $i ?>">&nbsp;Drag Area &nbsp;</div>
				</div>
	...
	<!-- Service id to update flow -->
	<input type="hidden" id="serviceID" value="<?php echo $service_id; ?>"  class="form-control" autocomplete="off" /> 
...
$(function(){
	...
	//var val=document.getElementById('lastdiv');
	//$("#"+val).css(
       // {"right": "10px"});
...
function Closegetquediv(opt,que,i){
...
function Closegetoptdiv(opt,que,i){
...
function setConnection(src, trg, conn){
	...
	if(lastID==''){
		...
		var getjs_status= getCookie('js_status');

		if(getjs_status=="null"){
			confirm("First click on the enable editing button");
		}
	}	*/
	/* target object type */
...
function setQuestion(){
		...
		var $serv_id=document.getElementById('service_id');
		alert($serv_id);alert($cat_id);
		document.getElementById('category').value=$cat_id;
		document.getElementById('service').value=$serv_id;*/
...
function removeConnection(tid, co){
	...
	if(confirm("Parent and child connection will also remove from database. Do you want to continue?")){
		...
		$('#optClose'+tid).css({"display":"inline-block","margin-top": "-19px","margin-right": "-15px"});
		$('#queClose'+tid).css({"display":"inline-block","margin-top": "-19px","margin-right": "-15px"});
		/*if(pid==""){
...
setTimeout(function(){
	$("#editloder").hide();
	$("#editFlow").fadeIn(1000);
},5000);


$("#editFlow").click(function(){
	$("#editFlow").text("Enabled");
});
