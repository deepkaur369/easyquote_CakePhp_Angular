...
function getImage(){
	...
	/*var imgdat=$('#immmg').val();
	alert(imgdat);
	if(imgdat==""){
	alert("ok");
		//document.getElementById('p1').style.display="inline-block";
		document.getElementById('p1').innerHTML="Image must Required";
...
function getError(){
	...
	$(document).ready(function(){
		document.getElementById("getScroll").click();
	});
}
</script>
...
<div class="col-lg-12">
	<section class="panel">
		...
		<div class="panel-body">
			...
			<?php echo $this->Form->input('user_id',['value' => $this->request->session()->read('id'),'type'=>"hidden",'label'=>false,'class'=>"form-control",'placeholder'=>""]); ?>
			...
			<input type="hidden"  id="immmg" name="screenshot" value=""/>
			<div class="form-group">
				<label id="scrshot" class="col-sm-3 control-label">Screen Shot<sup>*</sup></label>
				...
				<div  class="col-lg-6 cropHeaderWrapper">
					...
					<div id="pp1" style="color:red;display:none;">This field is required.</div>
				</div>
...
<a id="getScroll" href="#pp1"></a>
<?php if(isset($error)){
	echo '<script>getError(); </script>';
}?>