...
<div id="topBar">
	...
	<div style="width:50%;float:left;text-align:right">
		<a onclick="saveService('no')" href="#" id="saveService" class="btn btn-success"> Save Flow </a>
		<a onclick="saveService('yes')" href="#" id="save_Service" class="btn btn-success"> Save and New </a>
	</div>
...
<!--##################### Stack for Questions #############################-->
			...
			<div style="width:20%!important;float:left!important;">
				...
				?>
				<div style="top:20px; left:570px;" class="w q" id="<?php echo $i ?>">
				...
				<?php   
					...
					<div class="w q" ondrag="getPositions(this)" style="visibility:hidden;" id="<?php echo $i ?>">
						...
						<input type="hidden" id="style_<?php echo $i ?>" value="" />
							...
							<a style="color:white;" id="add<?php echo $i ?>" href="javascript:getClk(<?php echo $i ?>)" >Add Questions</a>
...
<!--######################### Stack for Options ##########################-->	
	...
	<?php 
		...
		foreach($questions as $question){
			$i = $question->id+99999999;
			?>
			
			<?php foreach($question->option_questions as $oq){ 
				...
				<script>
					/*$(document).ready(function(){
						jsPlumb.ready(function (){
							jsPlumb.connect({ source: "<?php echo $oq->option_id == 0 ?50:$oq->option_id+99999999; ?>", target: "<?php echo $oq->question_id+99999999 ?>" });
						});
					});*/
				</script>
			...
			<div class="w q" <?php echo $question->style; ?> ondrag="getPositions(this)" id="<?php echo $i ?>">
				...
				<input type="hidden" id="style_<?php echo $i ?>" value="<?php echo $question->style; ?>" />
					...
					<a style="color:white;display:none;"  id="add<?php echo $i ?>" href="javascript:getClk(<?php echo $i ?>)" >Add Questions</a>
					
					
					<!-- Update Button -->
				<span style="display:block;color:white;" id="pree<?php echo $i ?>">Q:&nbsp;</span>
				<a style="color:white;" href="#question" data-original-title="Question" data-content="<?php echo $question->question; ?>" data-placement="right" data-trigger="hover" class="popovers" style="display:none" id="edit<?php echo $i ?>" onclick="setID(<?php echo $i ?>,'question'),editQuestion(<?php echo $i ?>);" data-toggle="modal" data-target="#question" ><?php echo $question->title; ?></a>
			...
			?>
				...
				<script>
					/*$(document).ready(function(){
						...
						jsPlumb.ready(function (){
							jsPlumb.connect({ source: "<?php echo $option->question_id+99999999; ?>", target: "<?php echo $option->id+99999999; ?>" });
						});
					});*/
				</script>
				
				
				<div ondrag="getPositions(this)" <?php echo $option->style; ?>  class="w optiond_db" id="<?php echo $k ?>">
						
						<!-- hidden container -->
						<input type="hidden"  id="type<?php echo $k ?>" value="option" />
						<input type="hidden" id="last_insert_id_<?php echo $k ?>" value="<?php echo $option->id; ?>" />
						<input type="hidden" id="parent_id_<?php echo $k ?>" value="" />
						<input type="hidden" id="style_<?php echo $k ?>" value="<?php echo $option->style; ?>" />
					
						<a href="#option" id="clickMe_<?php echo $k ?>" onclick="setID(<?php echo $k ?>,'option')" data-toggle="modal" data-target="#option" ></a>
						
					<!-- Add Button -->
						<a id="add<?php echo $k ?>" style="display:none;" href="javascript:getClick(<?php echo $k ?>,'dummy')" >Add Options</a>
					<!-- Update Button -->
					
					<span style="color:black" id="pree<?php echo $k ?>">O:&nbsp;</span>	
					<a data-original-title="Option" data-content="<?php echo $option->name; ?>" data-placement="right" data-trigger="hover" class="popovers" id="edit<?php echo $k ?>" href="javascript:getClick(<?php echo $k ?>,'getdata')" ><?php echo $option->title; ?> </a>
						
						
						<div class="ep" style="color:white" id="drag<?php echo $k ?>">&nbsp;Drag Area &nbsp;</div>
				</div>
...
function getPositions(obj){
	...
	$('#xx').text("X: "+top);
	$('#yy').text("Y: "+left);
	var style = 'style="margin-top:'+top+'px";margin-left:"'+left+'px;"';
